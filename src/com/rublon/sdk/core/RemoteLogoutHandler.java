...
abstract public class RemoteLogoutHandler {
	 ...
	 */
	protected static final String FIELD_LOGOUT_USER_ID = "userId";
	...
	public void handle() {
		...
		if (input != null && input.length() > 0) {
			...
			if (!RublonSignature.verifyData(input, getRublon().getSecretKey(), getHTTPRequestHeader(RESTClient.HEADER_NAME_SIGNATURE))) {
				...
				String[] requiredFields = new String[] { FIELD_LOGOUT_ACCESS_TOKEN, FIELD_LOGOUT_USER_ID, FIELD_LOGOUT_DEVICE_ID };
				...
				// All OK
				logoutUser(message.optString(FIELD_LOGOUT_USER_ID), message.optInt(FIELD_LOGOUT_DEVICE_ID));
	 ...
	 */
	abstract protected void logoutUser(String userId, int deviceId);
